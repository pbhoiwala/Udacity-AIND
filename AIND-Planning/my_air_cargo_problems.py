...
class AirCargoProblem(Problem):
    ...
    def h_ignore_preconditions(self, node: Node):
        ...
        # node.state is our current state
        # we need to look at every goal in the self.goal and count the actions
        # that will accomplish the goal
        kb = PropKB()
        kb.tell(decode_state(node.state, self.state_map).pos_sentence())
        min_actions = [clause for clause in self.goal if clause not in kb.clauses]
        return len(min_actions)
